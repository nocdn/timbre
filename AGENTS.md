...
- Keep transcription model controls standardized across providers:
  - Keep each transcription provider settings panel ordered as: API key, `Streaming` toggle if supported, model picker, `VAD silence threshold` if the selected streaming model supports it, language input if supported, then help text.
  ...
  - Providers or streaming models with no VAD/silence threshold support should not show a `VAD silence threshold` control. When present, it should be a `NumberBox` in seconds, with range/default/step coming from `TranscriptionProviderCatalog`.
  ...
  - Keep provider capabilities centralized in `TranscriptionProviderCatalog`: display name, supported models, streaming compatibility, VAD silence threshold support, language behavior, upload limits, and defaults. Keep `TranscriptionModelCatalog` only as a compatibility facade for model-only call sites.
...
- Transcript insertion is shared by normal and streaming dictation through `TextInsertionService.InsertTextAsync`; it is not clipboard paste. Normal/final transcript insertion should use `TextInsertionMode.Auto`, which first tries UI Automation direct insertion via `ValuePattern.SetValue`, then falls back to Unicode `SendInput` typing. Streaming chunk insertion should use `TextInsertionMode.PreferUnicodeTyping` because chunks are small and caret stability matters more than direct insertion speed. Direct UIA insertion must only be treated as successful when the focused element also supports `TextPattern` and the caret can be moved/collapsed to the end after `SetValue`, otherwise chunks can insert out of order: the first chunk may set text while leaving the caret at the beginning, and later chunks typed via fallback will appear before it. Streaming chunk diagnostics in `DictationController` should log provider, chunk index, length, preview, and completion to make ordering/caret issues visible in `%LOCALAPPDATA%\Timbre\logs\`.
