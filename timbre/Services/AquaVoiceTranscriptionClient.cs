...
public sealed class AquaVoiceTranscriptionClient : ITranscriptionClient
{
    private static readonly Uri Endpoint = new("https://api.aquavoice.com/api/v1/audio/transcriptions");
    private const string DefaultModel = "avalon-v1-en";

    private readonly HttpClient _httpClient;

    public AquaVoiceTranscriptionClient(HttpClient httpClient)
    {
    ...
    public async Task<string> TranscribeAsync(byte[] audioBytes, string apiKey, string model, string? language, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new TranscriptionException("The AquaVoice API key is missing.", false);
        }
        ...
        try
        {
            ...
            DiagnosticsLogger.Info(
                $"AquaVoice transcription request starting. Endpoint={Endpoint}, Model={resolvedModel}, AudioBytes={audioBytes.Length}, Language='{resolvedLanguage}'.");

            using var response = await _httpClient.SendAsync(request, cancellationToken);
            var responseBody = await response.Content.ReadAsStringAsync(cancellationToken);
            DiagnosticsLogger.Info(
                $"AquaVoice transcription response received. Status={(int)response.StatusCode} {response.StatusCode}, BodyLength={responseBody.Length}.");
            ...
            {
                throw new TranscriptionException("AquaVoice returned an empty transcription.", false);
            }
        ...
        {
            throw new TranscriptionException("The transcription request could not reach AquaVoice.", true, null, exception);
        }
    }
    ...
    private static string ExtractErrorMessage(string responseBody, int statusCode)
    {
        ...
        return $"AquaVoice returned HTTP {statusCode}.";
    }
